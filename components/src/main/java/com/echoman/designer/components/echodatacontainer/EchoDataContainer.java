 ...
 * @author Dave Athlon
 */
public class EchoDataContainer extends JLabel implements MouseMotionListener, MouseListener,
        ActionListener, IEchoComponent {

    /**
     * Holds the node data instance.
     * @see #setNodeData(EchogetDataContainerNodeData node)
     */
    private EchoDataContainerNodeData nodeData;
    private transient EchoDataContainerNode node;
    ...
    private TitledBorder tb= new TitledBorder(new DashedBorder(Color.green, false), "", TitledBorder.CENTER, TitledBorder.BELOW_TOP, Font.getFont("Arial"), Color.GRAY);
     ...
     *
     */
    public EchoDataContainer(EchoDataContainerNodeData nodeData, int index, JPanel dropPanel) {
        ...
        //Ticket #438
        //setName("DataContainer" + index);
        setName("DataContainer");
        setSize(96, 48);
        setOpaque(false);
        setBackground(null);
        setBorder(tb);
    ...
    public void setNode(EchoBaseNode node) {
        this.node = (EchoDataContainerNode) node;
    }
     ...
     * @param node
     */
    public void setNodeData(EchoDataContainerNodeData node) {
        nodeData = node;
    }
    ...
    private void checkContainers() {
        if ((!mouseDragging) && (!nodeData.getLoadingForm())) {
            DataContainerManager.checkContainerContainers(nodeData,nodeData.getDesignerPage().getCompList());
            nodeData.updateContainerComponents();
            nodeData.fixZOrder();
            dropPanel.repaint();
        }
    }
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        form.setLastMouseX(e.getX());
        form.setLastMouseY(e.getY());
    }
    ...
    private void multiSelectComponents() {
        ...
        //    if ((compList.get(i) != this) && (rect.contains(((JComponent) compList.get(i).getComponent()).getLocation()))) {
        //        compList.get(i).getComponent().getNode().addSelectedNode(true);
        //    }
        //}
    }
