 ...
 * @author david.morin
 */
public class ValidationsTableCriteriaModel extends ValidationsCriteriaModel{
     ...
     * @param rowCount
     */
    public ValidationsTableCriteriaModel(String sql, Object[] columnNames, int rowCount) {
        super(sql, columnNames, rowCount);

        if (!SQLBuilder.EMPTY_STRING.equals(sql)) {
            Boolean validQuery = setQuery(sql);
            if (!validQuery) {
